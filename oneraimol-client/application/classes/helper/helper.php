 ...
 */
    class Helper_Helper {
        ...
        public static function batch_no($category = '') {
            $seed = ORM::factory('systemsetting')->find();
            ...
            if($category == Constants_Category::FINISHED_GOODS) {
                $bs = $seed->fg_seed;
                $catype = 'FG';
                $seed->fg_seed++;
            }
            else if($category == Constants_Category::BASE_OIL) {
                $bs = $seed->baseoil_seed;
                $catype = 'BOT';
                $seed->baseoil_seed++;
            }
            else if($category == Constants_Category::ADDITIVES) {
                $bs = $seed->additives_seed;
                $catype = 'ADD';
                $seed->additives_seed++;
            }

            $str = strtoupper($catype) . '-' . date("Y") . '-';
